...
class CNN_GCN(nn.Module):
    ...
    def __init__(self, drug_label, nfeat, block, num_blocks, hout=1, wout=1):
        ...
        self.linear = nn.Linear(215040, 1010)  # Adjusted to match the actual output from the preceding layers
...
## Changes made here
    def forward(self, x, y):
        ...
        x = x.view(x.size(0), -1)
        print("Shape before linear:", x.shape)  # Debugging shape before linear layer
        # Check the size here and adjust your linear layer accordingly
        x = F.relu(self.linear(x)) # Make sure self.linear's input features match x's size here
